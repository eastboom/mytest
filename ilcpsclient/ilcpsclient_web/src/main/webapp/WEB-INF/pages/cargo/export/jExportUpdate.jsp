...
<script language="JavaScript">
    $(document).ready(function(){
    	// 调用函数，给mRecordTable表格动态添加行
		// addTRRecord("mRecordTable", "id", "productNo", "cnumber", "grossWeight", "netWeight", "sizeLength", "sizeWidth", "sizeHeight", "exPrice", "tax");
    	
    	/*
    	   实现方法： 
    	   1. 从jExportList.jsp 列表选择一条报运单，进入jExportUpdate.jsp页面  (已经完成)
    	   2. 页面回显报运的商品
    	      	方案1： 页面body遍历报运商品。&lt;c:forEach&gt;
    			方案2： 页面发送ajax请求，ajax请求封装的就是报运的商品的json格式
    			               例如：[{"id":"","productNo":"","cnumber":"5",,,,,},{}]
    	*/
    	$.ajax({
    		// 请求后台地址
    		url : "${ctx}/cargo/exportAction_exportProductsJson",
    		// 请求类型
    		type : "post",
    		// 请求数据
    		data : {"id":$("#id").val()},
    		...
    		success : function(jsonData){
    			...
    			if (jsonData) {
    				for(var i=0; i<jsonData.length; i++){
    					...
    					var ep = jsonData[i];
    					// 调用js函数，动态添加商品数据行
    					addTRRecord("mRecordTable", ep.id, ep.productNo, ep.cnumber, ep.grossWeight, ep.netWeight, ep.sizeLength, ep.sizeWidth, ep.sizeHeight, ep.exPrice, ep.tax);
    				}
    			}
    		},
    		error : function(){
    			$.messager.alert("提示","ajax请求异常！","warning");
    		}
    	});
  ...
  <div class="textbox-title">
   ...
   修改出口报运
	...
	<div class="listTablew">
		<table id="mRecordTable" class="commonTable_main" cellSpacing="1">
